...
export function mergeHabitWithDayCompletion(habit, completion, options) {
  ...
  if (type === 'numeric') {
    ...
    const done = current >= target || completion?.isCompleted === true;
    return { ...habit, current, target, completed: done };
  }
...
/**
 * Streak ending at today or yesterday (if today not logged yet).
