...
/**
 * Streak ending at today or yesterday (if today not yet logged), like habit streaks.
 * @param {Map<string, boolean>} meaningfulByKey
 * @param {string} todayKey
 */
export function computeDashboardCurrentStreak(meaningfulByKey, todayKey) {
  if (!meaningfulByKey?.size) return 0;
  let anchor = todayKey;
  if (!meaningfulByKey.get(todayKey)) {
    const y = parseDateKey(todayKey);
    y.setDate(y.getDate() - 1);
    const yk = toDateKey(y);
    if (!meaningfulByKey.get(yk)) return 0;
    anchor = yk;
  }
  let n = 0;
...
/**
 * Longest run of consecutive meaningful days within sorted keys.
